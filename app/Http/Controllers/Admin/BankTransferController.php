...
class BankTransferController extends Controller
{
    ...
    {
        $accounts = CashAccount::with('outlet')->active()->orderBy('name')->get()->groupBy('outlet_id');
        $outlets = Outlet::active()->orderBy('name')->get();

        return view('admin.bank-transfers.create', compact('accounts', 'outlets'));
    }
    ...
    private function generateTransactionNumber(CashAccount $account, string $type, ?string $transactionDate = null): string
    {
        ...
        if ($lastTx) {
            $lastNumber = (int) substr($lastTx->transaction_number, -4);
            $newNumber = $lastNumber + 1;
        } else {
            $newNumber = 1;
        }
