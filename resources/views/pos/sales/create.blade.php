...
<div class="h-full flex flex-col md:flex-row overflow-hidden bg-gray-900" id="posApp">
    ...
    <div class="flex-1 flex flex-col min-w-0 bg-gray-900 border-r border-gray-800 relative">
        
        <!-- Header: Search & Categories -->
        <div class="flex-none p-4 md:p-6 space-y-4 bg-gray-900 z-10">
            <!-- Alert Session -->
            @if(!$activeSession)
            <div class="bg-red-500/10 border border-red-500/50 text-red-200 px-4 py-3 rounded-xl flex items-center shadow-lg backdrop-blur-sm">
                <svg class="w-6 h-6 mr-3 text-red-500" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z"/>
                </svg>
                <div>
                    <strong class="font-bold">Sesi Tertutup!</strong> Silakan buka shift terlebih dahulu untuk bertransaksi.
                </div>
            </div>
            @endif
            ...
            <div class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-4">
                <template v-for="product in filteredProducts" :key="product.id">
                    <button @click="addToCart(product)" 
                        ...
                        <!-- Image / Placeholder -->
                        <div class="aspect-[4/3] bg-gray-700/50 rounded-xl mb-3 flex items-center justify-center relative overflow-hidden group-hover:bg-gray-700 transition">
                            <!-- Pattern bg -->
                            ...
                            <!-- Icon -->
                            <svg class="w-10 h-10 text-gray-600 group-hover:text-indigo-400 transition transform group-hover:scale-110 duration-300" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M16 11V7a4 4 0 00-8 0v4M5 9h14l1 12H4L5 9z"/>
                            </svg>

                            <!-- Price Tag Overlay -->
                            <div class="absolute top-2 right-2 bg-gray-900/90 backdrop-blur text-indigo-400 text-xs font-bold px-2 py-1 rounded-lg border border-gray-700 shadow-sm">
                        ...
                        <div class="flex-1 flex flex-col">
                            <h4 class="text-sm font-semibold text-gray-100 group-hover:text-indigo-300 transition line-clamp-2 leading-tight mb-1" v-text="product.name"></h4>
    ...
    <div class="w-full md:w-[400px] xl:w-[450px] bg-gray-900 flex flex-col border-l border-gray-800 shadow-2xl z-20">
        ...
        <div class="flex-none p-5 border-b border-gray-800 bg-gray-900 shadow-sm z-20">
            ...
            <div v-if="selectedCustomer" class="mt-2 flex items-center justify-between text-xs px-1">
                ...
                <span class="text-gray-400">Poin: <span class="text-white">@{{ formatNumber(selectedCustomer.loyalty_points || 0) }}</span></span>
            </div>
        </div>
            ...
            <template v-if="cart.length > 0">
                <div v-for="(item, index) in cart" :key="index" 
                    ...
                    <div class="flex flex-col items-center justify-between bg-gray-900 rounded-lg p-1 w-8">
                        <button @click="increaseQty(index)" class="w-6 h-6 flex items-center justify-center text-gray-400 hover:text-white hover:bg-gray-700 rounded transition">
                            <svg class="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4v16m8-8H4"/></svg>
                        </button>
                        <span class="text-sm font-bold text-white select-none">@{{ item.quantity }}</span>
                        <button @click="decreaseQty(index)" class="w-6 h-6 flex items-center justify-center text-gray-400 hover:text-white hover:bg-gray-700 rounded transition">
                    ...
                    <div class="flex-1 min-w-0">
                        <div class="flex justify-between items-start mb-1">
                            ...
                            <span class="text-sm font-bold text-white whitespace-nowrap">@{{ formatNumber(item.subtotal) }}</span>
                        </div>
                        ...
                        <div class="flex items-center gap-3">
                            ...
                            <button @click="removeFromCart(index)" class="text-xs text-red-500 hover:text-red-400 flex items-center gap-1 px-2 py-1 rounded hover:bg-red-500/10 transition ml-auto">
                                <svg class="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16"/></svg>
                                Hapus
                            </button>
                        </div>
                        <div v-if="item.notes" class="mt-2 text-xs text-amber-500 italic bg-amber-500/10 p-1.5 rounded border border-amber-500/20">
                            "@{{ item.notes }}"
                        </div>
                    </div>
                </div>
        ...
        <!-- Checkout Section -->
        <div class="flex-none bg-gray-900 border-t border-gray-800 p-5 shadow-[0_-4px_20px_rgba(0,0,0,0.4)] z-30">
            <!-- Summary -->
            <div class="space-y-2 mb-4 text-sm">
                ...
                <div class="flex justify-between items-end pt-2">
                    ...
                    <span class="text-3xl font-bold text-white tracking-tight">Rp @{{ formatNumber(totalAmount) }}</span>
                </div>
            </div>
    ...
    <!-- Success Modal -->
    <div v-if="showSuccessModal" class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm" style="display: none;" :style="{ display: showSuccessModal ? 'flex' : 'none' }">
        <div class="bg-gray-800 rounded-2xl shadow-2xl max-w-sm w-full p-6 text-center border border-gray-700 animate-[bounceIn_0.3s_ease-out]">
            ...
            <div class="bg-gray-900/50 rounded-xl p-4 mb-6 border border-gray-700/50">
                <div class="flex justify-between items-center mb-1">
                    ...
                    <span class="text-xl font-bold text-white">Rp @{{ lastSale ? formatNumber(lastSale.total_amount) : 0 }}</span>
                </div>
            </div>

            <div class="space-y-3">
                <button @click="printReceipt" class="w-full py-3.5 bg-indigo-600 hover:bg-indigo-700 text-white rounded-xl font-bold flex items-center justify-center gap-2 transition shadow-lg shadow-indigo-600/20">
                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17 17h2a2 2 0 002-2v-4a2 2 0 00-2-2H5a2 2 0 00-2 2v4a2 2 0 002 2h2m2 4h6a2 2 0 002-2v-4a2 2 0 00-2-2H9a2 2 0 00-2 2v4a2 2 0 002 2zm8-12V5a2 2 0 00-2-2H9a2 2 0 00-2 2v4h10z" />
                    </svg>
                    CETAK STRUK
                </button>
                <button @click="closeSuccessModal" class="w-full py-3.5 bg-gray-700 hover:bg-gray-600 text-white rounded-xl font-semibold transition">
                    Transaksi Baru
                </button>
            </div>
        </div>
