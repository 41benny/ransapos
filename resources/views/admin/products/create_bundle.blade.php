...
<div class="max-w-6xl">
    ...
    <form action="{{ route('admin.products.store') }}" method="POST" id="bundleForm">
        @csrf
                    ...
                    <div>
                        ...
                        <div class="mt-4 space-y-3">
                            ...
                            <div id="outlet-selector-wrap" class="hidden pt-1">
                                <label for="pos_outlet_ids" class="block text-sm font-medium text-gray-700 mb-1">Pilih outlet</label>
                                <select name="pos_outlet_ids[]" id="pos_outlet_ids" multiple
                                    ...
                                    @foreach($outlets as $outlet)
                                        <option value="{{ $outlet->id }}" {{ in_array($outlet->id, old('pos_outlet_ids', [])) ? 'selected' : '' }}>
                                            {{ $outlet->name }}
                                        </option>
                                    @endforeach
                                </select>
                            ...
                            <label class="flex items-center gap-3 cursor-pointer">
                                <input type="checkbox" name="is_available_all_users" value="1"
                                    ...
                                    <tbody id="bundle-components-body">
                                        ...
                                        @foreach($oldComponents as $index => $component)
                                            <tr class="component-row border-b border-gray-200 last:border-b-0">
                                                <td class="px-4 py-3">
                                                    <select name="bundle_components[{{ $index }}][component_product_id]" class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-transparent">
                                                ...
                                                <td class="px-4 py-3">
                                                    <input type="text" name="bundle_components[{{ $index }}][uom]" value="{{ $component['uom'] ?? '' }}"
                                                        class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-transparent" placeholder="pcs">
                                                </td>
...
<script>
    document.addEventListener('DOMContentLoaded', function () {
        ...
        function bindAutoUnit(row) {
            ...
            const unitInput = row.querySelector('input[name*="[uom]"]');
        ...
        function addComponentRow() {
            ...
            row.innerHTML = `
                <td class="px-4 py-3">
                    <select name="bundle_components[${componentIndex}][component_product_id]" class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-transparent">
                        ${makeOptionsHtml()}
                    </select>
                </td>
                <td class="px-4 py-3">
