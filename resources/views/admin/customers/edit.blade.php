            ...
            <div class="bg-white rounded-xl shadow-md p-6">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                    ...
                    <div>
                        ...
                        <input type="text" value="{{ $customer->member_since ? $customer->member_since->format('d M Y') : '-' }}" disabled
                               class="w-full px-4 py-2 border border-gray-300 rounded-lg bg-gray-100">
                    </div>
                    ...
                    <div>
                        ...
                        <input type="date" name="birth_date" value="{{ old('birth_date', $customer->birth_date ? $customer->birth_date->format('Y-m-d') : '') }}"
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                    ...
                    <div>
                        ...
                        <select name="member_tier"
                            ...
                            <option value="gold" {{ old('member_tier', $customer->member_tier) == 'gold' ? 'selected' : '' }}>Gold</option>
                            <option value="platinum" {{ old('member_tier', $customer->member_tier) == 'platinum' ? 'selected' : '' }}>Platinum</option>
                        </select>
                        @error('member_tier')
                            <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                        @enderror
                ...
                <h3 class="text-lg font-semibold text-gray-800 mb-4 pb-2 border-b">Customer Statistics</h3>
                <div class="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
                    <div class="bg-blue-50 rounded-lg p-4">
                        <p class="text-xs text-gray-600 mb-1">Total Spending</p>
                        <p class="text-lg font-bold text-blue-600">Rp {{ number_format($customer->total_spending, 0, ',', '.') }}</p>
                    </div>
                    <div class="bg-green-50 rounded-lg p-4">
                        <p class="text-xs text-gray-600 mb-1">Transactions</p>
                        <p class="text-lg font-bold text-green-600">{{ $customer->total_transactions }}</p>
                    </div>
                    <div class="bg-purple-50 rounded-lg p-4">
                        <p class="text-xs text-gray-600 mb-1">Loyalty Points</p>
                        <p class="text-lg font-bold text-purple-600">{{ number_format($customer->loyalty_points) }}</p>
                    </div>
                    <div class="bg-amber-50 rounded-lg p-4">
                        <p class="text-xs text-gray-600 mb-1">Avg Transaction</p>
                        <p class="text-lg font-bold text-amber-600">Rp {{ number_format($customer->average_transaction, 0, ',', '.') }}</p>
                    </div>
                </div>
