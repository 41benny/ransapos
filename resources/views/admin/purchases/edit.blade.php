...
@section('page-subtitle', 'Perbarui data pembelian')
    ...
    <div class="max-w-6xl space-y-6">
        ...
        <div class="bg-white rounded-xl shadow-sm border border-gray-100">
            <div class="p-6 border-b border-gray-100">
                ...
                <p class="text-sm text-gray-500 mt-1">{{ $purchase->purchase_number }}</p>
            </div>
            <div class="p-6 grid grid-cols-1 md:grid-cols-3 gap-6">
                ...
                <div>
                    ...
                    <select name="outlet_id" required class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div class="space-y-4">
                        ...
                        <div>
                            ...
                            <input type="number" name="tax_amount" value="{{ old('tax_amount', $purchase->tax_amount) }}" min="0" step="0.01"
                        ...
                        <div>
                            ...
                            <input type="number" name="discount_amount" value="{{ old('discount_amount', $purchase->discount_amount) }}" min="0" step="0.01"
                        ...
                        <div>
                            ...
                            <textarea name="notes" rows="3" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500">{{ old('notes', $purchase->notes) }}</textarea>
                        </div>
                    </div>
        ...
        <div class="flex items-center justify-end space-x-3">
            <a href="{{ route('admin.purchases.show', $purchase) }}" class="px-6 py-3 border border-gray-300 text-gray-700 rounded-lg hover:bg-gray-50 transition">
                Batal
            </a>
...
function addItem(existingItem = null) {
    ...
    row.innerHTML = `
        ...
        <td class="px-4 py-3">
            <input type="number" name="items[${itemIndex}][quantity]" value="${quantity}" min="0.01" step="0.01" required
                   onchange="calculateItemSubtotal(${itemIndex})"
                   class="w-24 px-3 py-2 border border-gray-300 rounded-lg text-sm" id="qty-${itemIndex}">
        </td>
        <td class="px-4 py-3">
            <input type="number" name="items[${itemIndex}][unit_price]" value="${unitPrice}" min="0" step="0.01" required
                   onchange="calculateItemSubtotal(${itemIndex})"
                   class="w-32 px-3 py-2 border border-gray-300 rounded-lg text-sm" id="price-${itemIndex}">
        </td>
        <td class="px-4 py-3">
            <input type="number" name="items[${itemIndex}][discount_amount]" value="${discount}" min="0" step="0.01"
                   onchange="calculateItemSubtotal(${itemIndex})"
