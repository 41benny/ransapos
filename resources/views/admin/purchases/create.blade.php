...
@section('page-subtitle', 'Buat pesanan pembelian baru untuk pengadaan stok barang')
...
<div class="mx-auto w-full max-w-7xl animate-in fade-in slide-in-from-bottom-2 duration-500">
    ...
    <div class="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-6">
        <div>
            <h1 class="text-2xl font-normal text-slate-800 tracking-tight">Buat Pembelian</h1>
            <p class="text-xs font-normal text-slate-500 mt-0.5">Isi detail pesanan pembelian untuk sinkronisasi inventaris</p>
        </div>
    ...
    @if(session('error'))
        <div class="mb-6 rounded-xl bg-rose-50 border border-rose-100 p-4 flex items-start gap-3 text-rose-600 animate-in slide-in-from-top-2">
    ...
    @if($errors->any())
        <div class="mb-6 rounded-xl bg-amber-50 border border-amber-100 p-4 flex flex-col gap-2 text-amber-700 animate-in slide-in-from-top-2 text-xs">
    ...
    <form action="{{ route('admin.purchases.store') }}" method="POST" id="purchaseForm" class="space-y-6">
            ...
            <div class="p-6">
                <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                    <div class="flex flex-col gap-1.5">
                        ...
                        <select name="outlet_id" required class="ui-input w-full px-4 py-2.5 text-sm font-normal bg-white border border-slate-200 rounded-xl focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 transition-all shadow-sm">
                    ...
                    <div class="flex flex-col gap-1.5">
                        ...
                        <select name="supplier_id" required class="ui-input w-full px-4 py-2.5 text-sm font-normal bg-white border border-slate-200 rounded-xl focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 transition-all shadow-sm">
                    ...
                    <div class="flex flex-col gap-1.5">
                        ...
                        <input type="date" name="purchase_date" value="{{ old('purchase_date', date('Y-m-d')) }}" required
                               class="ui-input w-full px-4 py-2.5 text-sm font-normal bg-white border border-slate-200 rounded-xl focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 transition-all shadow-sm">
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="ui-card bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden">
            <div class="p-4 border-b border-slate-100 bg-slate-50/50 flex justify-between items-center">
                ...
                <button type="button" onclick="addItem()" class="ui-btn ui-btn-primary inline-flex items-center gap-2 rounded-lg bg-indigo-600 px-4 py-2 text-xs font-normal text-white shadow-sm transition-all hover:bg-indigo-700 active:scale-95">
                    <i class="fas fa-plus"></i>
                    <span>TAMBAH ITEM</span>
                </button>
            </div>
            <div class="overflow-x-auto">
                <table class="ui-table min-w-full divide-y divide-slate-200" id="itemsTable">
            ...
            <div class="p-4 bg-slate-50/50 border-t border-slate-100">
                <p class="text-[9px] text-slate-400 italic font-normal uppercase tracking-wider">* Klik tombol "Tambah Item" untuk menambah baris produk baru.</p>
            </div>
        </div>
        ...
        <div class="flex flex-col lg:flex-row gap-6">
            {{-- Notes & Adjustments --}}
            <div class="ui-card w-full lg:w-2/3 bg-white rounded-2xl shadow-sm border border-slate-200 p-6 space-y-6">
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div class="flex flex-col gap-1.5">
                        ...
                        <input type="text" name="tax_amount" value="{{ old('tax_amount', 0) }}" inputmode="decimal" data-currency-input="1"
                               ...
                               class="ui-input w-full px-4 py-2.5 text-sm font-normal bg-white border border-slate-200 rounded-xl focus:ring-2 focus:ring-indigo-500 transition-all shadow-sm">
                    </div>
                    <div class="flex flex-col gap-1.5">
                ...
                <div class="flex flex-col gap-1.5">
                    ...
                    <textarea name="notes" rows="3" placeholder="Contoh: Pembayaran tempo 30 hari..."
                              class="ui-input w-full px-4 py-2.5 text-sm font-normal bg-white border border-slate-200 rounded-xl focus:ring-2 focus:ring-indigo-500 transition-all shadow-sm">{{ old('notes') }}</textarea>
                </div>
            </div>

            {{-- Calculations --}}
            <div class="ui-card w-full lg:w-1/3 bg-white rounded-2xl shadow-sm border border-slate-200 p-6 overflow-hidden flex flex-col justify-between">
                <div class="space-y-4">
                ...
                <div class="mt-8 pt-6 border-t border-slate-100 flex justify-between items-end">
                    <div class="flex flex-col">
                        <span class="text-[9px] font-normal text-slate-400 uppercase tracking-[0.2em] leading-none mb-1">Total Bayar</span>
                        <span class="text-2xl font-normal text-indigo-600 tracking-tight leading-none tabular-nums" id="displayTotal">Rp 0</span>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="flex items-center justify-end gap-3 pt-6 border-t border-slate-100">
            ...
            <button type="submit" id="submitPurchaseBtn"
                class="ui-btn ui-btn-primary inline-flex items-center gap-2 rounded-xl bg-slate-900 px-10 py-3.5 text-xs font-normal text-white shadow-lg transition-all hover:bg-slate-800 active:scale-95 uppercase tracking-widest">
                <i class="fas fa-check text-[10px]"></i>
                <span>Simpan Transaksi Pembelian</span>
            </button>
        </div>
    </form>
</div>
    ...
    row.innerHTML = `
        <td class="px-5 py-4">
            <div class="flex flex-col gap-1.5">
                <input type="text" id="product-input-${currentIndex}" list="${productMasterListId}"
                       ...
                       class="ui-input w-full px-4 py-2 text-sm font-normal bg-white border border-slate-200 rounded-xl focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 transition-all shadow-sm">
        ...
        <td class="px-5 py-4">
            <input type="number" name="items[${currentIndex}][quantity]" value="${quantity}" min="0.01" step="0.01" required
                   ...
                   class="ui-input w-full h-10 px-3 text-right text-sm font-normal bg-white border border-slate-200 rounded-xl focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 transition-all shadow-sm tabular-nums" id="qty-${currentIndex}">
        </td>
        <td class="px-5 py-4">
            <input type="text" name="items[${currentIndex}][unit_price]" value="${unitPrice}" inputmode="decimal" data-currency-input="1" required
                   onchange="calculateItemSubtotal(${currentIndex})"
                   class="ui-input w-full h-10 px-3 text-right text-sm font-normal bg-white border border-slate-200 rounded-xl focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 transition-all shadow-sm tabular-nums" id="price-${currentIndex}">
        </td>
        <td class="px-5 py-4">
            <input type="text" name="items[${currentIndex}][discount_amount]" value="${discountAmount}" inputmode="decimal" data-currency-input="1"
                   onchange="calculateItemSubtotal(${currentIndex})"
                   class="ui-input w-full h-10 px-3 text-right text-sm font-normal bg-white border border-slate-200 rounded-xl focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 transition-all shadow-sm tabular-nums" id="discount-${currentIndex}">
        </td>
