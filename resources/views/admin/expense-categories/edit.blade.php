...
<div class="container mx-auto px-4 py-6">
    ...
    <div class="mb-6">
        ...
        <p class="text-gray-600 mt-1">Update expense category information</p>
    </div>
        ...
        <form action="{{ route('admin.expense-categories.update', $expenseCategory) }}" method="POST">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <input type="text" name="code" value="{{ old('code', $expenseCategory->code) }}" required
                           class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent">
                    @error('code')
                    <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                    @enderror
                ...
                <div>
                    ...
                    <select name="parent_id"
                        ...
                        <option value="{{ $parent->id }}"
                                {{ old('parent_id', $expenseCategory->parent_id) == $parent->id ? 'selected' : '' }}>
                            {{ $parent->name }}
                        </option>
                        @endforeach
                    </select>
                ...
                <div>
                    ...
                    <select name="coa_account_id"
                        ...
                        <option value="{{ $account->id }}"
                                {{ old('coa_account_id', $expenseCategory->coa_account_id) == $account->id ? 'selected' : '' }}>
                ...
                <div>
                    ...
                    <input type="number" name="order" value="{{ old('order', $expenseCategory->order) }}" min="0"
                           class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent">
                    @error('order')
                    <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                    @enderror
