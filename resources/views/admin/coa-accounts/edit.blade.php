...
<div class="container mx-auto px-4 py-6">
    ...
    <div class="mb-6">
        <div class="flex items-center space-x-2 text-sm text-gray-600 mb-2">
            ...
            <span>/</span>
            <span class="text-gray-900">{{ $coaAccount->code }}</span>
        </div>
        <h1 class="text-2xl font-bold text-gray-900">Edit Akun: {{ $coaAccount->name }}</h1>
    </div>
    ...
    <div class="bg-white rounded-lg shadow max-w-2xl">
        <form action="{{ route('admin.coa-accounts.update', $coaAccount) }}" method="POST">
            @csrf
            @method('PUT')
            ...
            <div class="p-6 space-y-6">
                ...
                <div>
                    ...
                    <input type="text" 
                           id="code" 
                           name="code" 
                           value="{{ old('code', $coaAccount->code) }}"
                ...
                <div>
                    ...
                    <input type="text" 
                           id="name" 
                           name="name" 
                           value="{{ old('name', $coaAccount->name) }}"
                ...
                <div>
                    ...
                    <select id="type" 
                        ...
                        <option value="asset" {{ old('type', $coaAccount->type) == 'asset' ? 'selected' : '' }}>Asset (Aset)</option>
                        <option value="liability" {{ old('type', $coaAccount->type) == 'liability' ? 'selected' : '' }}>Liability (Kewajiban)</option>
                        <option value="equity" {{ old('type', $coaAccount->type) == 'equity' ? 'selected' : '' }}>Equity (Modal)</option>
                    </select>
                    @error('type')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                ...
                <div>
                    ...
                    <input type="text" 
                           id="group" 
                           name="group" 
                           value="{{ old('group', $coaAccount->group) }}"
                ...
                <div>
                    <label class="flex items-center space-x-2">
                        <input type="checkbox" 
                               ...
                               name="is_active" 
                               value="1"
                               {{ old('is_active', $coaAccount->is_active) ? 'checked' : '' }}
