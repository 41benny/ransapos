    ...
    <div class="p-6 border-b border-gray-100 no-print">
        <form method="GET" action="{{ route('admin.reports.shifts.index') }}" class="grid grid-cols-1 md:grid-cols-5 gap-4">
            ...
            <div class="md:col-span-5 flex space-x-2">
                ...
                <a href="{{ route('admin.reports.shifts.index', ['tab' => request('tab', 'penjualan')]) }}" class="px-5 py-2.5 bg-white border border-amber-300 hover:bg-amber-50 text-amber-900 rounded-full transition">
                    Reset
                </a>
                <a href="{{ route('admin.reports.shifts.export', array_merge(request()->query(), ['format' => 'xlsx'])) }}" class="px-5 py-2.5 bg-emerald-50 border border-emerald-200 hover:bg-emerald-100 text-emerald-700 rounded-full transition">
                    Export Excel
                </a>
                <a href="{{ route('admin.reports.shifts.export', array_merge(request()->query(), ['format' => 'pdf'])) }}" class="px-5 py-2.5 bg-white border border-gray-300 hover:bg-gray-50 text-gray-700 rounded-full transition">
                    Export PDF
                </a>
