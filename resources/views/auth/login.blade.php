...
<body
    ...
    <div class="relative flex h-screen w-full flex-col overflow-hidden">
        ...
        <div class="relative z-10 flex flex-1 items-center justify-center p-4">
            <div
                class="w-full max-w-[440px] rounded-2xl bg-[#1e1411]/95 dark:bg-[#1e1411]/95 p-8 shadow-[0_8px_32px_rgba(0,0,0,0.5)] border border-white/10 backdrop-blur-sm">
                <!-- Logo Area -->
                <div class="mb-8 flex flex-col items-center text-center">
                    <div
                        class="mb-4 flex h-16 w-16 items-center justify-center rounded-full bg-gradient-to-tr from-primary to-orange-600 shadow-lg shadow-orange-900/50">
                ...
                <form action="{{ route('login.post') }}" method="POST" class="flex flex-col gap-5">
                    ...
                    @if(session('error'))
                        <div
                            class="bg-red-500/10 border border-red-500/20 text-red-400 px-4 py-3 rounded-xl text-sm text-center">
                            {{ session('error') }}
                        </div>
                    @endif
                    ...
                    <label class="flex flex-col gap-2">
                        <span class="text-xs font-bold uppercase tracking-wider text-white/70 ml-1">Email / Staff
                            ID</span>
                        <div class="relative flex items-center">
                            <span class="absolute left-4 text-white/40 material-symbols-outlined">badge</span>
                            <input name="email" value="{{ old('email') }}"
                                class="h-14 w-full rounded-xl border border-white/10 bg-white/5 pl-12 pr-4 text-white placeholder-white/30 focus:border-primary focus:bg-white/10 focus:outline-none focus:ring-1 focus:ring-primary transition-all duration-200"
                    ...
                    <label class="flex flex-col gap-2">
                        ...
                        <div class="relative flex items-center">
                            ...
                            <input name="password"
                                class="h-14 w-full rounded-xl border border-white/10 bg-white/5 pl-12 pr-12 text-white placeholder-white/30 focus:border-primary focus:bg-white/10 focus:outline-none focus:ring-1 focus:ring-primary transition-all duration-200"
                                placeholder="••••••••" type="password" required />
                            <button
                                class="absolute right-4 flex items-center justify-center text-white/40 hover:text-white transition-colors"
                                type="button" onclick="togglePasswordVisibility()">
                                <span class="material-symbols-outlined" id="password-toggle-icon">visibility_off</span>
                            </button>
                        </div>
                    </label>
                    ...
                    <div class="flex justify-between items-center">
                        <label class="flex items-center gap-2 cursor-pointer">
                            <input type="checkbox" name="remember"
                                class="rounded border-white/10 bg-white/5 text-primary focus:ring-primary">
                            <span class="text-sm font-medium text-white/50">Remember me</span>
                        </label>
                        <a class="text-sm font-medium text-primary hover:text-orange-400 transition-colors"
                            href="#">Forgot Password?</a>
                    </div>

                    <!-- Login Button -->
                    <button type="submit"
                        class="mt-2 flex h-14 w-full items-center justify-center gap-2 rounded-xl bg-primary text-white shadow-lg shadow-orange-900/40 hover:bg-orange-600 hover:shadow-orange-900/60 active:scale-[0.98] transition-all duration-200">
                ...
                <div class="mt-8 text-center">
                    ...
                    <a class="mt-1 inline-flex items-center gap-1 text-xs font-medium text-white/50 hover:text-white transition-colors"
                        href="#">
